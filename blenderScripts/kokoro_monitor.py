    ...
    try:
        ...
        while not stop_event.is_set():
            try:
                ...
                elif command_type == "set_visual_state":
                    state_value = command_data.get("state")
                    conversation_id = command_data.get("conversation_id") # Extract conversation_id for this command
                    if state_value and conversation_id:
                        bpy.app.timers.register(
                            lambda s=state_value, c_id=conversation_id:
                            print(f"BLENDER_MONITOR: Main thread: Received visual state - {s}. Conversation ID: {c_id}")
                        )
                    else:
                        missing_fields = []
                        if not state_value: missing_fields.append("'state'")
                        if not conversation_id: missing_fields.append("'conversation_id'")
                        print(f"BLENDER_MONITOR: Error - {', '.join(missing_fields)} not provided for 'set_visual_state' command. Data: {command_data}")
                elif command_type == "face_tracking_update":
                    # print(f"DEBUG: Received face_tracking_update: {command_data}") # Can be verbose
                    norm_x = command_data.get("face_center_x_normalized")
                    norm_y = command_data.get("face_center_y_normalized")
                    face_detected = command_data.get("face_detected", False) # Default to False if missing

                    if face_detected and norm_x is not None and norm_y is not None:
                        if blink and hasattr(blink, 'update_eye_target_location'):
                            # print(f"DEBUG: Queueing update_eye_target_location with norm_x={norm_x}, norm_y={norm_y}")
                            # Define default scale factors here or pass them from command_data if needed
                            scale_x_factor = 1.0 # Adjust as needed
                            scale_y_factor = 1.0 # Adjust as needed
                            bpy.app.timers.register(
                                lambda nx=norm_x, ny=norm_y, sx=scale_x_factor, sy=scale_y_factor:
                                safe_execute(blink.update_eye_target_location, nx, ny, scale_x=sx, scale_y=sy)
                            )
                        else:
                            print("Error: blink.py module or update_eye_target_location function not available.")
                    elif not face_detected:
                        # Optional: Handle case where face is explicitly not detected
                        # For example, reset eye target to a default position
                        if blink and hasattr(blink, 'update_eye_target_location'):
                            # print("DEBUG: Face not detected, queueing eye target reset (e.g., to 0,0).")
                            # bpy.app.timers.register(lambda: safe_execute(blink.update_eye_target_location, 0.0, 0.0))
                            pass # Or do nothing, let eyes stay where they were
                        else:
                            print("Error: blink.py module or update_eye_target_location function not available for reset.")
                    else:
                        print(f"Face tracking update missing norm_x or norm_y, or face_detected is false. Data: {command_data}")
