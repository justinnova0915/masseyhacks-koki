...
FACE_TRACKING_SUB_NAME = "kokoro-face-tracking-coordinates-sub" # New subscription name
...
FACE_TRACKING_TOPIC_NAME = "kokoro-face-tracking-coordinates" # New topic name
...
def face_tracking_callback(message: pubsub_v1.subscriber.message.Message) -> None:
    """Handles incoming face tracking coordinate messages."""
    logging.debug(f"Received face tracking message ID: {message.message_id}") # Changed to debug for potentially high frequency
    try:
        data_str = message.data.decode("utf-8")
        payload = json.loads(data_str)
        # logging.info(f"Face tracking payload: {payload}") # Can be verbose

        # Add command_type for kokoro_monitor.py to identify this message
        payload["command_type"] = "face_tracking_update"
        
        send_command_to_blender(payload)
        
        message.ack()
    except Exception as e:
        logging.error(f"Error processing face tracking message: {e}", exc_info=True)
        message.nack()
...
if __name__ == "__main__":
    ...
    face_tracking_subscriber, face_tracking_future = None, None # New
    ...
    try:
        ...
        logging.info("Starting Face Tracking Subscriber...")
        face_tracking_subscriber, face_tracking_future = subscribe_to_topic(
            GCP_PROJECT_ID, FACE_TRACKING_TOPIC_NAME, FACE_TRACKING_SUB_NAME, face_tracking_callback, credentials
        )
        if not face_tracking_future:
            logging.error("Failed to start face tracking subscriber.")
            # Potentially exit or handle error
        ...
        if face_tracking_future:
            face_tracking_future.cancel()
            try:
                face_tracking_future.result(timeout=10)
            except TimeoutError:
                logging.warning("Timeout waiting for face tracking subscriber to shut down.")
            except Exception as e_sub:
                logging.warning(f"Error during face tracking subscriber shutdown: {e_sub}")
        if face_tracking_subscriber:
            face_tracking_subscriber.close()
